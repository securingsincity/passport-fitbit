...
/**
 * Retrieve user summar from Fitbit.
 *
 * This function constructs a normalized profile, with the following properties:
 *
 *   - `id`
 ...
 * @api protected
 */
Strategy.prototype.userActivityToday = function(token, tokenSecret, params, done) {
  var today = new Date();
  var todaysDate = today.getFullYear() +"-" + (today.getMonth() + 1) + "-"+ today.getDate();
  this._oauth.get('https://api.fitbit.com/1/user/-/activities/date/'+todaysDate+'.json', token, tokenSecret, function (err, body, res) {
    if (err) { return done(new InternalOAuthError('failed to fetch user activity today', err)); }
